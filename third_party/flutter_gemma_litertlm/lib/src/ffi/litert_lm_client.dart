...
class LiteRtLmFfiClient {
  ...
  Future<void> initialize({
    ...
    try {
      ...
      // Document links point at a read-only descriptor owned by the app.
      // Reopening /proc/self/fd through a filename can fail under Android FUSE.
      // Give LiteRT-LM a dup: the engine owns it, the app retains the original.
      int? documentFd;
      if (Platform.isAndroid && await FileSystemEntity.isLink(modelPath)) {
        final target = await Link(modelPath).target();
        final match = RegExp(r'^/proc/self/fd/(\d+)$').firstMatch(target);
        if (match != null) documentFd = int.parse(match.group(1)!);
      }
      final Pointer<LiteRtLmEngineSettings> settings;
      if (documentFd != null) {
        final dup = DynamicLibrary.open('libc.so')
            .lookupFunction<Int32 Function(Int32), int Function(int)>('dup');
        final ownedFd = dup(documentFd);
        if (ownedFd < 0) throw StateError('Cannot duplicate model document descriptor');
        settings = b.settingsFromFileDescriptor(ownedFd, backendPtr.cast(),
        ...
        settings = b.litert_lm_engine_settings_create(
          modelPathPtr.cast(), backendPtr.cast(),
          visionBackendPtr == nullptr ? nullptr : visionBackendPtr.cast(),
          audioBackendPtr == nullptr ? nullptr : audioBackendPtr.cast());
      }
      gemmaLog(
